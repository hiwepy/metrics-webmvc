...
public class GetMappingCounterAspect extends AbstractMetricMethodAspect<GetMapping, Counter> {
	...
	public GetMappingCounterAspect(final MetricRegistry metricRegistry) {
		this(metricRegistry, false);
	}
	
	public GetMappingCounterAspect(final MetricRegistry metricRegistry, boolean absolute) {
		super(metricRegistry, absolute, ANNOTATION, METHOD_FILTER);
	}

	@Override
	protected Object invoke(ProceedingJoinPoint joinPoint, Counter counter, GetMapping annotation) throws Throwable {
		counter.inc();
		return joinPoint.proceed();
	}

	@Override
	protected Counter buildMetric(MetricRegistry metricRegistry, String metricName, GetMapping annotation) {
