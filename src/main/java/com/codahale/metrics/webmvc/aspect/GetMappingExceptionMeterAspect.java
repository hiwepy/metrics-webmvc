...
public class GetMappingExceptionMeterAspect extends AbstractMetricMethodAspect<GetMapping, Meter> implements Ordered {
	...
	public GetMappingExceptionMeterAspect(final MetricRegistry metricRegistry) {
		this(metricRegistry, false);
	}
	
	public GetMappingExceptionMeterAspect(final MetricRegistry metricRegistry, boolean absolute) {
		super(metricRegistry, absolute, ANNOTATION, METHOD_FILTER);
	}

	@Override
	protected Object invoke(ProceedingJoinPoint joinPoint, Meter meter, GetMapping annotation) throws Throwable {
		try {
	...
	@Override
	protected Meter buildMetric(MetricRegistry metricRegistry, String metricName, GetMapping annotation) {
