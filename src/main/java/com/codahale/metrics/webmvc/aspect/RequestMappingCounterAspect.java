...
public class RequestMappingCounterAspect extends AbstractMetricMethodAspect<RequestMapping, Counter> implements Ordered {
	...
	public RequestMappingCounterAspect(final MetricRegistry metricRegistry) {
		this(metricRegistry, false);
	}
	
	public RequestMappingCounterAspect(final MetricRegistry metricRegistry, boolean absolute) {
		super(metricRegistry, absolute, ANNOTATION, METHOD_FILTER);
	}

	@Override
	protected Object invoke(ProceedingJoinPoint joinPoint, Counter counter, RequestMapping annotation) throws Throwable {
		counter.inc();
		return joinPoint.proceed();
	}

	@Override
	protected Counter buildMetric(MetricRegistry metricRegistry, String metricName, RequestMapping annotation) {
